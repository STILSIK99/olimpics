...
void solve()
{
    ...
    cin >> n;
    ll ans = 0, x, y;
    map<int, int> dom, guest;
    forin (0, n) {
        cin >> x >> y;
        if (dom.count (x) == 0) dom[x] = 1;
        else ++dom[x];
        if (guest.count (y) == 0) guest[y] = 1;
        else ++guest[y];
    }
    forina (dom.begin(), dom.end()) {
        if (guest.count (i->first) == 0) continue;
        ans += i->second * (guest[i->first]);
    }
    cout << ans;
}
