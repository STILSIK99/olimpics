...
map<int, bool> simple_numbers (ll n)
{
    map<int, bool> ans;
...
void solve()
{
    ...
    forin (0, n) {
        cin >> mas[i];
        if (mas[i] == 1) start[i] = true;
        if (simple_n.count (mas[i]) != 0) start[i] = true;
    }
    ll ans = 0;
    while (start.size() != 0) {
        ll ind = start.begin()->first;
        if (ind - e > -1) {
            ll mn = mas[ind - e] * mas[ind], next = ind;
            while (mn == 1 || (start.count (mn * mas[next]) != 0)) {
                ++ans;
                next += e;
            }
            if (simple_n.count (mas[next]) != 0) {
                ll mn = mas[next];
                next -= e;
                while (simple_n.count (mn) != 0) {
                    ++ans;
                    next -= e;
                    mn *= mas[next];
                }
